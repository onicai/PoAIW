...
# top off cycles
CURRENT_BALANCE=$(icp canister status challenger_ctrlb_canister -e $NETWORK_TYPE 2>&1 | grep "Cycles:" | awk '{gsub("_", ""); print $2}')
...
if [ $(echo "$NEED_CYCLES_THRESHOLD > 0" | bc) -eq 1 ]; then
    CANISTER_ID=$(icp canister status challenger_ctrlb_canister -e $NETWORK_TYPE --id-only)
