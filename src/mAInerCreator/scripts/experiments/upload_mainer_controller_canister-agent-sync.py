...
Run with:
    ...
    python -m scripts.upload_mainer_controller_canister.py
"""
...
from ..ic_py_canister import get_canister, get_agent, run_icp_command
...
def main() -> int:
    ...
    response = agent_instance.query_raw(
        canister_id,
        canister_method,
        method_args_encoded
    )
    ...
    method_args = []
    method_args_encoded = encode(method_args)
    response = agent_instance.update_raw(
        canister_id,
        canister_method,
        method_args_encoded
    )
    ...
    for i, chunk in enumerate(generate_chunks(wasm_bytes, chunksize)):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                method_args = [{'type': Types.Vec(Types.Nat8), 'value': chunk}]
                method_args_encoded = encode(method_args)
                response = agent_instance.update_raw(
                    canister_id,
                    canister_method,
