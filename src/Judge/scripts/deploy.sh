...
if [ "$NETWORK_TYPE" = "ic" ]; then
    if [ "$SUBNET" = "none" ]; then
        dfx deploy judge_ctrlb_canister -m reinstall --yes --network $NETWORK_TYPE
    else
        dfx deploy judge_ctrlb_canister -m reinstall --yes --network $NETWORK_TYPE --subnet $SUBNET
    fi
else
    dfx deploy judge_ctrlb_canister -m reinstall --yes --network $NETWORK_TYPE
fi
