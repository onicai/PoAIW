...
persistent actor class JudgeCtrlbCanister() = this {
    ...
    // recurring send-cycles timer). `sentBy` is recorded on the CyclesTransaction.
    private func sendCyclesToGameStateInternal(sentBy : Principal) : async Types.AddCyclesResult {
        ...
        try {
            ...
            if (currentCyclesBalance < CYCLES_AMOUNT_TO_GAME_STATE_CANISTER + MIN_CYCLES_BALANCE) {
                D.print("Judge: sendCyclesToGameStateInternal - balance too low: balance " # debug_show(currentCyclesBalance) # " < required " # debug_show(CYCLES_AMOUNT_TO_GAME_STATE_CANISTER + MIN_CYCLES_BALANCE) # " (send " # debug_show(CYCLES_AMOUNT_TO_GAME_STATE_CANISTER) # " + keep MIN " # debug_show(MIN_CYCLES_BALANCE) # ") - skipping. sentBy " # debug_show(sentBy));
                return #Ok({ added = false; amount = 0 });
            };

            let gameStateCanisterActor = actor (GAME_STATE_CANISTER_ID) : Types.GameStateCanister_Actor;
            D.print("Judge: sendCyclesToGameStateCanister gameStateCanisterActor = " # Principal.toText(Principal.fromActor(gameStateCanisterActor)));
            D.print("Judge: sendCyclesToGameStateCanister - CYCLES_AMOUNT_TO_GAME_STATE_CANISTER: " # debug_show(CYCLES_AMOUNT_TO_GAME_STATE_CANISTER));
            Cycles.add<system>(CYCLES_AMOUNT_TO_GAME_STATE_CANISTER);
            
            D.print("Judge: sendCyclesToGameStateCanister - calling gameStateCanisterActor.addCycles");
            let addCyclesResponse = await gameStateCanisterActor.addCycles();
            D.print("Judge: sendCyclesToGameStateCanister - addCyclesResponse: " # debug_show(addCyclesResponse));
            switch (addCyclesResponse) {
                case (#Err(error)) {
                    D.print("Judge: sendCyclesToGameStateCanister - addCyclesResponse FailedOperation: " # debug_show(error));
                ...
                case (#Ok(addCyclesResult)) {
                    D.print("Judge: sendCyclesToGameStateCanister - addCyclesResult: " # debug_show(addCyclesResult));
            ...
            D.print("Judge: sendCyclesToGameStateCanister - Failed to send cycles to Game State: " # Error.message(e));      
            // Store the failed attempt
            let transactionEntry : Types.CyclesTransaction = {
                amountAdded : Nat = CYCLES_AMOUNT_TO_GAME_STATE_CANISTER;
                newOfficialCycleBalance : Nat = Cycles.balance();
            ...
            cyclesTransactionsStorage := List.push<Types.CyclesTransaction>(transactionEntry, cyclesTransactionsStorage);
            return #Err(#Other("Judge: sendCyclesToGameStateCanister - Failed to send cycles to Game State: " # Error.message(e)));
        };
    };
    ...
    //-------------------------------------------------------------------------
    // Admin RBAC Storage
    // Needed because, post-SNS decentralization, controller-gated endpoints are
    // only callable by NNS/SNS root — role-based admin is the remaining path.
    //-------------------------------------------------------------------------
    ...
    private func removeAdminRole(principal : Text) : Bool {
        switch (adminRoleAssignmentsStorage.get(principal)) {
            ...
            case (?assignment) {
                let _ = adminRoleAssignmentsStorage.remove(principal);
                return true;
            };
        };
    };
    ...
    // Check admin permissions. Controllers automatically have all permissions;
    // an #AdminUpdate role also satisfies an #AdminQuery requirement.
    ...
    private func judgeChallengeResponseDoIt_(submissionEntry : Types.ChallengeResponseSubmission) : async Types.JudgeChallengeResponseResult {
        ...
        if (SEND_CYCLES_TO_LLM) {
            ...
            // LLM cycles cap: skip funding when the LLM is already above MIN_CYCLES_BALANCE.
            // Fail-open: if the cached balance cannot be read, fund anyway so judging never stalls.
            let fundLlm : Bool = try {
                switch (await llmCanister.get_cycle_balance()) {
                    case (#Ok(cycleBalanceRecord)) {
                        if (cycleBalanceRecord.cycle_balance > MIN_CYCLES_BALANCE) {
                            D.print("Judge: judgeChallengeResponseDoIt_ - LLM " # debug_show(llmCanisterPrincipal) # " cap reached: balance " # debug_show(cycleBalanceRecord.cycle_balance) # " > cap " # debug_show(MIN_CYCLES_BALANCE) # " - skipping deposit");
                            false;
                        } else {
                            D.print("Judge: judgeChallengeResponseDoIt_ - LLM " # debug_show(llmCanisterPrincipal) # " under cap: balance " # debug_show(cycleBalanceRecord.cycle_balance) # " <= cap " # debug_show(MIN_CYCLES_BALANCE) # " - funding");
                            true;
                        };
                    };
                    case (#Err(error)) {
                        D.print("Judge: judgeChallengeResponseDoIt_ - LLM " # debug_show(llmCanisterPrincipal) # " cap: get_cycle_balance Err " # debug_show(error) # " - funding anyway (fail-open)");
                        true;
                    };
                };
            } catch (e) {
                D.print("Judge: judgeChallengeResponseDoIt_ - LLM " # debug_show(llmCanisterPrincipal) # " cap: get_cycle_balance trapped: " # Error.message(e) # " - funding anyway (fail-open)");
                true;
            };
            if (fundLlm) {
                try {
...
// Timer
    ...
    // ----------------------------------------------------------------------
    // Recurring send-cycles timer: periodically drains excess cycles down to
    // the GameState canister via the existing fixed-chunk sendCyclesToGameStateInternal.
    // Transient (in-memory) — must be re-armed after every upgrade.
    ...
    private func triggerSendCyclesToGameState() : async () {
        D.print("Judge: triggerSendCyclesToGameState - triggered (Cycles.balance() = " # debug_show(Cycles.balance()) # ")");
        try {
            let result = await sendCyclesToGameStateInternal(Principal.fromActor(this));
            D.print("Judge: triggerSendCyclesToGameState - result: " # debug_show(result));
        } catch (e) {
            D.print("Judge: triggerSendCyclesToGameState - Error: " # Error.message(e));
        };
    };
    ...
    private func startSendCyclesTimer() : async Types.AuthRecordResult {
        ...
        ignore setTimer<system>(#seconds 5,
            func () : async () {
                D.print("Judge: setTimer (send-cycles)");
                let id = recurringTimer<system>(#seconds sendCyclesPeriodInSeconds, triggerSendCyclesToGameState);
                D.print("Judge: Successfully started send-cycles timer with id = " # debug_show(id));
    ...
    private func stopSendCyclesTimer() : async Types.AuthRecordResult {
        switch (sendCyclesTimerId) {
            case (?id) {
                D.print("Judge: Stopping send-cycles timer with id = " # debug_show(id));
                Timer.cancelTimer(id);
    ...
    system func postupgrade() {
        ...
        // Reconstruct the admin-role map from stable storage
